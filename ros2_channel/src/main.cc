...
class Ros2ChannelModule : public aimrt::ModuleBase {
 public:
  ...
  bool Initialize(aimrt::CoreRef core) override {
    ...
    // 注册发布者类型
    aimrt::channel::RegisterPublishType<std_msgs::msg::String>(publisher_);
    // 初始化订阅者
    subscriber_ = core_.GetChannelHandle().GetSubscriber("test_topic");
    // 订阅topic 并指明回调函数
    aimrt::channel::Subscribe<std_msgs::msg::String>(
  ...
  // 订阅者收到topic时的回调函数
  void DataCallback(const std::shared_ptr<const std_msgs::msg::String> &data) {
    AIMRT_INFO("Receive new ros event, data:\n{}",
               std_msgs::msg::to_yaml(*data));
  }
